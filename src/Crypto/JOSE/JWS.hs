...
newJWSHeaderProtected :: (ProtectionOptionality p) => Alg -> JWSHeader p
...
--
makeJWSHeader
  :: forall e m p. (MonadError e m, AsError e, ProtectionOptionality p)
  => JWK
  -> m (JWSHeader p)
makeJWSHeader k = do
  let
...
instance (HasParams a, ProtectionOptionality p) => FromJSON (Signature p a) where
...
instance (HasParams a, ProtectionOptionality p) => ToJSON (Signature p a) where
...
-- 'ProtectionOptionality' type, and the header record type.
--
...
-- or 'Unprotected'.
--
type GeneralJWS = JWS [] ProtectionOptional

-- | A JWS with one signature, which uses the
-- /flattened serialisation/.  Headers may be 'Protected'
-- or 'Unprotected'.
--
type FlattenedJWS = JWS Identity ProtectionOptional
...
--
type CompactJWS = JWS Identity ProtectedOnly
...
instance (HasParams a, ProtectionOptionality p) => FromJSON (JWS [] p a) where
  parseJSON v =
...
instance (HasParams a, ProtectionOptionality p) => FromJSON (JWS Identity p a) where
  parseJSON =
...
instance (HasParams a, ProtectionOptionality p) => ToJSON (JWS [] p a) where
  toJSON (JWS p [s]) = Types.insertToObject "payload" p (toJSON s)
  toJSON (JWS p ss) = object ["payload" .= p, "signatures" .= ss]

instance (HasParams a, ProtectionOptionality p) => ToJSON (JWS Identity p a) where
...
--
instance (HasParams a) => ToCompact (JWS Identity ProtectedOnly a) where
  toCompact (JWS p (Identity s@(Signature _ _ (Types.Base64Octets sig)))) =
    [ view recons $ signingInput s p
    , review Types.base64url sig
    ]

instance (HasParams a) => FromCompact (JWS Identity ProtectedOnly a) where
...
verifySig
  :: (HasJWSHeader a, HasParams a, ProtectionOptionality p)
  => Types.Base64Octets
  -> Signature p a
  -> JWK
  -> Either Error Bool
